...
async function refreshUnread(){
  try{
    ...
    window._mailUnread=snap.docs.filter(d=>{const m=d.data();return m.read===false&&m.type!=='friendreq'&&m.type!=='friendaccept'&&m.type!=='refreward'&&m.type!=='visit'&&m.type!=='marriage'&&m.type!=='marriage_accept';}).length;
    updateMailBadge();
  }catch(e){}
}
...
window.forgotPassword=async function(){
  ...
  if(!em||!em.includes('@')){notify('📧','Спершу введи свій email у поле вище');return;}
  try{
    await sendPasswordResetEmail(auth,em);
    notify('📧 Лист надіслано!','Перевір пошту '+em+' — там посилання для зміни пароля (і теку «Спам»).',7000);
  }catch(e){
    const m=(e.code==='auth/user-not-found')?'Такого email немає в грі':((e.code==='auth/invalid-email')?'Невірний email':e.message);
    notify('❌',m);
  }
};
...
function startDecay(){
  ...
  decayInt=setInterval(()=>{
    ...
    if(P.health==null)P.health=100;
    const neglect=((P.hunger||0)<20)+((P.thirst||0)<20)+((P.fun||0)<15);
    if(!P.sick){
...
function coinMul(){let m=(bonusActive('coin2')||bonusActive('vip'))?2:1;if(P&&P._clubBuffUntil&&P._clubBuffUntil>Date.now())m*=1.25;return m;}
function gainXP(n){
...
window.goPage=function(id){
  ...
  if(id==='settings'&&typeof refreshInstallUI==='function'){refreshInstallUI();if(typeof applyTheme==='function')applyTheme();const sb=$('sound-toggle-btn');if(sb&&P)sb.textContent=P.sound?'🔊 Звук: увімкнено':'🔇 Звук: вимкнено';}
...
window.act=function(type){
  ...
  switch(type){
    case'feed':
      ...
      P.hunger=cl((P.hunger||0)+25);P.hearts=(P.hearts||0)+20;gainXP(3);notify('🍖 Смачно!','+25🍖 +20❤️');
      showEmotion('eat',2600);trackTask('feedCnt');lifeTrack('feed');break;
    case'water':
      if((P.thirst||0)>=100){notify('💧 Напоєний!','');return;}
      P.thirst=cl((P.thirst||0)+30);P.hearts=(P.hearts||0)+10;gainXP(2);notify('💧 Освіжився!','+30💧 +10❤️');
...
function finishWalk(){
  ...
  coins=Math.round(coins*coinMul()*(typeof weatherToday==='function'?weatherToday().walkMul:1));
...
function trainCost(lv){return 30+lv*8;} // прогресія: 30, 38 ... 662 (рів.79); усі 3 навички ≈ 83 000 ❤️
...
const SHOP={
  ...
  bonuses:[
    ...
    {id:'b_vip',  icon:'👑',name:'VIP (24 год)',      desc:'XP+монети x2 на добу',cost:600,bonus:'vip',mins:1440},
...
async function loadMail(kind){
  ...
  const arr=snap.docs.map(d=>({_id:d.id,...d.data(),time:tsToStr(d.data().ts)}))
    .filter(m=>m.type!=='friendreq'&&m.type!=='friendaccept'&&m.type!=='refreward'&&m.type!=='visit'&&m.type!=='marriage'&&m.type!=='marriage_accept') // службові — окремо
...
function houseUpCost(lv){return Math.round(120*Math.pow(1.85,lv));} // 120,222,411,760,1407,2602,4814,8906,16476,30481
...
window.fishingAction=function(){
  ...
  let coins=0,parts=[];
  if(c.cMin!=null){coins=c.cMin+Math.floor(Math.random()*(c.cMax-c.cMin+1));coins=Math.round(coins*coinMul()*(typeof weatherToday==='function'?weatherToday().fishMul:1));P.coins=(P.coins||0)+coins;parts.push('🪙'+coins);}
...
window.plantCrop=function(key){
  ...
  P.coins-=c.cost;
  const gm=(typeof weatherToday==='function')?weatherToday().growMul:1;
...
window.visitFriend=function(){
  ...
  // бонус собі
  P.coins=(P.coins||0)+VISIT_BONUS.coins;P.butterflies=(P.butterflies||0)+VISIT_BONUS.bf;
  // надсилаємо «візит» другу — він отримає бонус при вході
  ...
  notify('🏡 Гарний візит!','+🪙'+VISIT_BONUS.coins+' 🦋'+VISIT_BONUS.bf+' тобі, і другу теж!',4500);
...
// ════════ ЕКСПЕДИЦІЇ ════════
const EXPEDITIONS=[
  {key:'forest',  icon:'🌲',name:'Похід у ліс',     dur:3600, energy:25,cMin:150, cMax:300, ing:2,        xp:30, desc:'1 година'},
  {key:'mountain',icon:'⛰️',name:'Гірська виправа',  dur:10800,energy:35,cMin:400, cMax:700, gem:1,        xp:60, desc:'3 години'},
  {key:'sea',     icon:'🌊',name:'Морська подорож',  dur:21600,energy:45,cMin:700, cMax:1200,bf:15,ing:3,   xp:100,desc:'6 годин'},
  {key:'treasure',icon:'🗺️',name:'Пошук скарбів',    dur:43200,energy:60,cMin:1500,cMax:2800,gem:2,bf:20,   xp:180,desc:'12 годин'},
];
let _expTicker=null;
...
window.startExpedition=function(key){
  ...
  if(typeof sickBlocks==='function'&&sickBlocks())return;
  if((P.energy||0)<e.energy){notify('😴','Замало енергії ('+e.energy+')');return;}
  P.energy=cl((P.energy||0)-e.energy);

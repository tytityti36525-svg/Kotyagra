...
async function refreshUnread(){
  try{
    ...
    window._mailUnread=snap.docs.filter(d=>{const m=d.data();return m.read===false&&m.type!=='friendreq'&&m.type!=='friendaccept';}).length;
    updateMailBadge();
  }catch(e){}
}
...
async function loadMail(kind){
  ...
  const arr=snap.docs.map(d=>({_id:d.id,...d.data(),time:tsToStr(d.data().ts)}))
    .filter(m=>m.type!=='friendreq'&&m.type!=='friendaccept') // службові — окремо

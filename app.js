...
function weMul(field){const w=worldEvent();return w[field]||1;}
function updateWorldEventUI(){
  const w=worldEvent();const b=$('world-event-banner');
  if(b){b.style.display='flex';b.innerHTML='<span class="we-ic">'+w.icon+'</span><div><b>Подія місяця: '+w.name+'</b><div class="we-desc">'+w.desc+'</div></div>';}
}
function xpMul(){return ((bonusActive('xp2')||bonusActive('vip'))?2:1)*weMul('xp');}
function coinMul(){let m=(bonusActive('coin2')||bonusActive('vip'))?2:1;if(P&&P._clubBuffUntil&&P._clubBuffUntil>Date.now())m*=1.25;return m*weMul('coin')*(1+skillEff('coinPct')/100);}
function gainXP(n){

...
window.act=function(type){
  ...
  switch(type){
    ...
    case'sleep':
      ...
      notify('💤 Спить!','Прокинеться через 2 хв (120 сек)');
      render();
      let sleepSecs=120; // 20 хв = 120 сек гри
      const sleepCountEl=$('sleep-countdown');
      clearTimeout(sleepTimer);
      const sleepTick=()=>{
        if(sleepSecs>0){
          if(sleepCountEl)sleepCountEl.textContent='⏱ '+Math.floor(sleepSecs/60)+'хв '+(sleepSecs%60)+'с';sleepSecs--;
    ...
    if(type==='players'){
      ...
      rl.innerHTML=players.slice(0,30).map((p,i)=>
        `<div class="rating-row"><span class="rrank ${i===0?'gold':i===1?'silver':i===2?'bronze':''}">${medals[i]||i+1}</span>
          <span class="rav">🐱</span>
          <div><div class="rname">${esc(p.catname||p.nickname||'?')}${p._id===uid?' 🌟':''}</div>
          <div class="rsub">@${esc(p.nickname||'?')} · Рів.${p.level||1} · 🌳${p.walkCoins||0}🪙</div></div>

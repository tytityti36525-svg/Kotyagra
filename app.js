...
async function loadClubData(){
  ...
  try{
    ...
    $('cl-members').innerHTML=order.map(m=>{
      ...
      let ctrl='';
      if(canUp)ctrl+='<button class="mrole-btn up" title="Підвищити" onclick="promoteMember(\''+mid+'\')">▲</button>';
      if(canDown)ctrl+='<button class="mrole-btn down" title="Понизити" onclick="demoteMember(\''+mid+'\')">▼</button>';
      if(canKick)ctrl+='<button class="mrole-btn kick" title="Виключити" onclick="kickMember(\''+mid+'\')">✕</button>';
...
window.kickMember=async function(mid){
  ...
  if(!confirm('Виключити '+(m.catname||m.nickname||'учасника')+' з клубу?'))return;
  try{

...
onAuthStateChanged(auth,async user=>{
  ...
  if(local){
    P=sanitizePlayer(JSON.parse(local));startGame();
    getDoc(doc(db,'players',uid)).then(s=>{
      if(s.exists()){const r=sanitizePlayer(s.data());if((r.level||1)>=(P.level||1)){P=r;render();renderPetPage();saveP();}}
    }).catch(()=>{});return;
  }
  try{
    ...
    if(s.exists()){P=sanitizePlayer(s.data());}
      ...
      P=mkPlayer(n,'Котик','cat');setDoc(doc(db,'players',uid),san(P)).catch(()=>{});}
  }catch(e){const n=(user.displayName||'Гравець').replace(/\s+/g,'_').slice(0,20);P=mkPlayer(n,'Котик','cat');}
  startGame();
});
...
function buildShowPage(){
  const emojis=getPetEmojis();
  $('show-cat-emoji').textContent=emojis[0];
  $('show-cat-name').textContent=P.catname||'Тваринка';
  ...
  cats.innerHTML=SHOW_CATS.map(sc=>{
    ...
    return `<div class="show-category ${sc.special?'show-special':''}">
      ...
      <button class="show-enter-btn" ${!canEnter?'disabled':''} onclick="enterShow('${sc.id}')">
        ${canEnter?'Взяти участь 🐾':'Потрібно '+sc.reqLvl+' рівень'}</button>
    </div>`;
  }).join('');
...
function buildShowLeaderboard(){
  ...
  const contenders=[
    ...
    {name:P.catname||'Ти',score:myScore,av:emojis[0]},
...
window.enterShow=function(catId){
  ...
  if(win){
    ...
    wiggle();
    notify('🏆 Перемога!',P.catname+' переміг на "'+sc.name+'"! +'+coins+'🪙');
    addLog(P.catname+' виграв "'+sc.name+'"! 🏆');
  }else{
    gainXP(8);
    ...
    cb.innerHTML=clubs.map(c=>`
      <div class="club-list-item" onclick="joinClub('${c._id}')">
        ...
        <div style="flex:1">
          ...
          <div class="cli-info">${c.memberCount||1} учасників · Рів.${c.level||1} · Засн. ${c.founded||'—'}</div>
...
async function loadClubData(){
  ...
  try{
    ...
    $('cl-name').textContent=c.name||'Клуб';
    const stars=Math.min(7,c.stars||1);
    $('cl-stars').textContent='★'.repeat(stars)+'☆'.repeat(Math.max(0,7-stars));
    $('cl-role-badge').textContent=isDirector?'👑 Директор':'👤 Учасник';
    $('cl-role-badge').className='club-role-badge '+(isDirector?'role-dir':'role-mem');
    // Info
    ...
    // Piggy
    $('cl-pc').textContent=formatNum(c.piggyCoins||0);
    $('cl-ph').textContent=formatNum(c.piggyHearts||0);
    // Buildings
    $('cl-blvl').textContent=c.buildingsLevel||1;
    $('ps-club').textContent='Клуб: '+c.name;
    // Director controls
    $('club-director-row').style.display=isDirector?'block':'none';
    // Show level-up button if enough XP
...
window.joinClub=async function(clubId){
  ...
  try{
    ...
    const banner=$('club-join-banner');if(banner){banner.style.display='block';setTimeout(()=>banner.style.display='none',4000);}
    notify('🎈 Ласкаво просимо!','Ти вступив до клубу '+snap.data().name+'!');
...
window.createClub=async function(){
  ...
  const iconSel=$('new-club-icon')?.value||CLUB_ICONS[0];
  if(!name){notify('📝 Введи назву','Назва обов\'язкова!');return;}
  if(name.length<3){notify('📝 Коротка назва','Мінімум 3 символи!');return;}
  if(P.clubId){notify('✅ Вже в клубі','Спочатку вийди з поточного!');return;}
  if((P.coins||0)<50){notify('🪙 Мало монет','Для створення клубу потрібно 🪙50!');return;}
  try{
    P.coins-=50;
    const ref=await addDoc(collection(db,'clubs'),{
      name,description:desc||'Наш чудовий клуб!',icon:iconSel,
    ...
    P.clubId=ref.id;saveP();
    notify('🏆 Клуб створено!','"'+name+'" — ти Директор! (-🪙50)');
  ...
  try{
    if(type==='players'){
      ...
      const players=snap.docs
        ...
        .filter(p=>p.nickname&&p.catname) // must have both
        .sort((a,b)=>((b.level||1)*1000+(b.butterflies||0))-((a.level||1)*1000+(a.butterflies||0)));
      if(!players.length){rl.innerHTML='<div class="loading-inline">Поки немає гравців</div>';return;}
      const emojis=getPetEmojis();
      rl.innerHTML=players.map((p,i)=>{
        ...
        return `<div class="rating-row ${isMe?'rating-row-me':''}" onclick="openPlayerProfile('${p._id}')">
          ...
          <div style="flex:1">
            <div class="rname">${esc(p.catname||'Тваринка')}${isMe?' 🌟':''}</div>
      ...
      rl.innerHTML=players.map((p,i)=>{
        ...
        return `<div class="rating-row ${isMe?'rating-row-me':''}" onclick="openPlayerProfile('${p._id}')">
          <span class="rrank ${i===0?'gold':i===1?'silver':i===2?'bronze':''}">${medals[i]||i+1}</span>
          <span class="rav">${av}</span>
          <div style="flex:1"><div class="rname">${esc(p.catname||'Тваринка')}${isMe?' 🌟':''}</div>
...
function startChat(){
  ...
  // Always reload from Firestore fresh
  if(chatUnsub){chatUnsub();chatUnsub=null;}
  box.innerHTML='<div class="loading-inline">Завантаження чату...</div>';
  try{
    // Order by ts, keep last 200 messages
    const q=query(collection(db,'chat'),orderBy('ts','asc'),limit(200));
    let firstLoad=true;
    chatUnsub=onSnapshot(q,snap=>{
      if(firstLoad){
        ...
        snap.docs.forEach(docSnap=>{
          ...
          addChatMsg(docSnap.id,d.nickname||'?',d.catname||'',d.text||'',d.uid===uid,d.ts);
        });
        ...
        snap.docChanges().forEach(ch=>{
          if(ch.type==='added'){
            const d=ch.doc.data();
            addChatMsg(ch.doc.id,d.nickname||'?',d.catname||'',d.text||'',d.uid===uid,d.ts);
            box.scrollTop=box.scrollHeight;
          }
        });
      }
...
function addChatMsg(docId,nickname,catname,text,isMine,ts){
  const box=$('chat-msgs');if(!box)return;
  // Avoid duplicates
  if(docId&&$('cmsg-'+docId))return;
  ...
  const displayName=catname
    ?`${esc(catname)} <span style="font-size:.6rem;opacity:.7">@${esc(nickname)}</span>`
    :`@${esc(nickname)}`;
...
window.sendChat=async function(){
  const inp=$('chat-in');const text=inp.value.trim();if(!text||text.length>300)return;
  inp.value='';
  try{
    ...
    trackTask('chatCount');
  }catch(e){console.error('sendChat:',e);inp.value=text;}
};
...
function renderTasks(){
  ...
  list.innerHTML=tasks.map(t=>{
    ...
    const done=P.tasks.completed[t.id]||(getProgressVal(t)>=t.goal);

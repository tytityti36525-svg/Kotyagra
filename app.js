...
const FB = {
  apiKey:"AIzaSyBaQY50Yi6VwuhM6Lat-xjFWdZdtcr4FsE",
...
window.doAuth=async function(){
  ...
  const authTimer=setTimeout(()=>{
    resetBtn();
    setErr('Firebase не відповідає (timeout). Перевір інтернет або спробуй пізніше.');
  },12000);
  try{
    if(authMode==='reg'){
      try{const nq=await getDocs(query(collection(db,'players'),where('nickname','==',nick),limit(1)));
        if(!nq.empty){clearTimeout(authTimer);setErr('Такий нікнейм вже зайнятий!');resetBtn();return;}}catch(e){}
    ...
    const msgs={
      ...
      'auth/network-request-failed':'Немає інтернету. Перевір з\'єднання.',
      'auth/unauthorized-domain':'Домен не авторизований у Firebase! Додай у Firebase Console → Authentication → Settings → Authorized domains.',
      'auth/operation-not-allowed':'Email/пароль вхід вимкнено у Firebase Console!',
      'auth/internal-error':'Внутрішня помилка Firebase.',
    };
    const msg=msgs[e.code]||('Помилка ('+e.code+'): '+e.message);
    setErr(msg);
  }
};
...
getRedirectResult(auth).then(async result=>{
  if(result&&result.user){
    ...
    try{
      ...
      if(!s.exists()){
        const n=(u.displayName||u.email||'Гравець').replace(/\W/g,'_').slice(0,20);

...
function sanitize(p){
  ...
  if(p.storyBaseIdx==null)p.storyBaseIdx=-1;
...
function renderGames(){renderRiddle();renderRoulette();renderMemory();renderRPS();renderLuckyBox();renderPuzzle();renderWordle();}
// ════════ КАМІНЬ-НОЖИЦІ-ПАПІР ════════
const RPS_MAX=5;
function renderRPS(){
...
// ════════ ІСТОРІЯ: ПОШУК МУРЧИКА (з 20 рівня) ════════
const STORY_LEVEL=20;
const STORY=[
  {icon:'😿',title:'Зникнення',track:'feed',goal:3,
   text:'Одного ранку твій найкращий друг — котик Мурчик — не прийшов на сніданок. На підвіконні лишилася тільки його улюблена іграшка… Збери сили в дорогу — добре погодуй себе.',
   reward:{coins:500,hearts:100}},
  {icon:'🐾',title:'Перший слід',track:'walk',goal:3,
   text:'Біля воріт ти помітив маленькі котячі сліди, що ведуть у місто. Пройдися вулицями — можливо, хтось бачив Мурчика.',
   reward:{coins:700,hearts:150}},
  {icon:'🎣',title:'Підказка біля річки',track:'fish',goal:5,
   text:'Старий рибалка біля річки каже: «Бачив рудого котика, що сидів отут і ловив рибу!» Полови й ти — може, знайдеш ще підказку.',
   reward:{coins:900,hearts:150}},
  {icon:'⛰️',title:'Гірська стежка',track:'expedition',goal:1,
   text:'Сліди ведуть у гори. Це небезпечно, але заради друга ти готовий на все. Вирушай в експедицію через перевал.',
   reward:{coins:1200,hearts:200,crystals:1}},
  {icon:'🌾',title:'Покинутий хутір',track:'harvest',goal:8,
   text:'На хуторі ти знаходиш записку: «Котика забрали до старого маєтку». Господиня хутора попросить допомогти із врожаєм в обмін на карту.',
   reward:{coins:1400,hearts:200}},
  {icon:'🏆',title:'Випробування',track:'showWin',goal:1,
   text:'Біля маєтку влаштували котячий конкурс — і тільки переможець потрапляє всередину. Покажи свого улюбленця на виставці й перемож!',
   reward:{coins:1800,hearts:300,crystals:1}},
  {icon:'🎉',title:'Возз\'єднання!',track:null,goal:0,
   text:'У найдальшій кімнаті маєтку ти знаходиш Мурчика — він був зачинений, але цілий і неушкоджений! Друзі нарешті разом. Мурчик муркоче й дарує тобі скарб на згадку про вашу пригоду. 🐱❤️🐱',
   reward:{coins:5000,hearts:1000,crystals:3}},
];
function storyTrackVal(track){return track?getProgress({track:track}):0;}
function storyEnsureBase(){
  // якщо базу ще не зафіксовано для поточного розділу — фіксуємо
  if(P.storyBaseIdx!==P.story){
    const ch=STORY[P.story];P.storyBase=ch&&ch.track?storyTrackVal(ch.track):0;P.storyBaseIdx=P.story;saveP();
  }
}
function renderStory(){
  const box=$('story-body');if(!box)return;
  if((P.level||1)<STORY_LEVEL){
    box.innerHTML='<div class="info-box-green">📖 Пригода «Пошук Мурчика» відкриється на <b>'+STORY_LEVEL+'</b> рівні. Зараз у тебе '+(P.level||1)+'.</div>';
    return;
  }
  if(P.story>=STORY.length){
    box.innerHTML='<div class="card" style="text-align:center"><div style="font-size:3rem">🐱❤️🐱</div><div class="sec-title">Історію завершено!</div><div class="fam-note">Ти знайшов Мурчика й пережив незабутню пригоду. Дякуємо, що грав! 🎉</div></div>';
    return;
  }
  storyEnsureBase();
  const ch=STORY[P.story];const last=P.story===STORY.length-1;
  ...
  let h='<div class="story-progress-top">Розділ '+(P.story+1)+' з '+STORY.length+'</div>';
  ...
  if(ch.track){
    ...
    const objName={feed:'Погодувати себе',walk:'Сходити на прогулянку',fish:'Зловити рибу',expedition:'Сходити в експедицію',harvest:'Зібрати врожай',showWin:'Перемогти на виставці'}[ch.track]||ch.track;
  ...
  else h+='<div class="story-note">Виконай ціль, щоб продовжити пригоду</div>';
  h+='</div>';
  box.innerHTML=h;
}
window.claimStory=function(){
  const ch=STORY[P.story];if(!ch)return;
  storyEnsureBase();
  const prog=ch.track?Math.max(0,storyTrackVal(ch.track)-P.storyBase):0;
  ...
  P.coins=(P.coins||0)+rw.coins;if(rw.hearts)P.hearts=(P.hearts||0)+rw.hearts;if(rw.crystals)P.crystals=(P.crystals||0)+rw.crystals;
  gainXP(30);
  P.story++;P.storyBaseIdx=-1; // наступний розділ зафіксує нову базу
  if(typeof sfx==='function')sfx('success');if(typeof spawnReaction==='function')spawnReaction(['📖','🐾','⭐','🎉']);
  notify(ch.icon+' '+ch.title,'Розділ завершено! +🪙'+rw.coins,4500);
  addLog('Історія: '+ch.title+' завершено');
...
function startPuzzleGame(){
  ...
  for(let i=0;i<120;i++){const adj=puzzleAdj(empty);const m=adj[Math.floor(Math.random()*adj.length)];t[empty]=t[m];t[m]=null;empty=m;}
...
function renderPuzzle(){
  ...
  box.innerHTML='<div class="puzzle-grid">'+_puzzle.tiles.map((v,i)=>
...
window.wordleEnter=function(){
  ...
  if(guess===_wordle.target){
    ...
    if(P.wordleDay!==dayKey()){
      P.wordleDay=dayKey();P.wordleSolved=true;

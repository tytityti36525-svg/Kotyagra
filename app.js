...
window.fishingAction=function(){
  ...
  const tot=FISH_CATCH.reduce((s,x)=>s+x.w*(bias[x.name]||1),0);
  function rollCatch(){let r=Math.random()*tot,c=FISH_CATCH[0];for(const f of FISH_CATCH){r-=f.w*(bias[f.name]||1);if(r<0){c=f;break;}}return c;}
  let c=rollCatch();
...
window.giftCardPick=function(cardId){
  ...
  if(list)list.innerHTML=P.friends.map(f=>'<button class="gc-friend" onclick="giftCardSend(\''+esc(String(f.id))+'\',\''+esc(String(f.nick||'друг'))+'\')">👤 '+esc(f.nick||'друг')+'</button>').join('');
...
window.giftCardSend=function(fid,fnick){
  ...
  P.cards[_giftCardId]=(P.cards[_giftCardId]||0)-1;
  if(typeof sendLetter==='function'){
    sendLetter({toUid:fid,toNick:fnick,subj:'🎴 Подарунок-картка!',
      body:(P.nickname||'Друг')+' дарує тобі картку: '+card.icon+' '+card.name+'!',
      type:'card_gift',icon:'🎴',extra:{cardId:String(_giftCardId)}}).catch(()=>{});
  }
  if(typeof sfx==='function')sfx('success');if(typeof spawnReaction==='function')spawnReaction(['🎴','🎁','💌']);
  notify('🎁 Подарунок надіслано!',fnick+' отримає картку '+card.icon+' '+card.name+' поштою');
  addLog('Подаровано картку: '+card.name+' → '+fnick);
  closeGiftCard();render();saveP();renderCards();
};
async function processCardGifts(){
  if(!P)return;
  try{
    const snap=await getDocs(query(collection(db,'mail'),where('toUid','==',String(uid)),limit(80)));
    const gifts=snap.docs.map(d=>({_id:d.id,...d.data()})).filter(m=>m.type==='card_gift'&&m.cardId);
    let got=[];
    for(const m of gifts){
      const card=allCards().find(c=>c.id===m.cardId);
      if(card){if(!P.cards)P.cards={};P.cards[m.cardId]=(P.cards[m.cardId]||0)+1;got.push(card.icon+' '+card.name);}
      try{await deleteDoc(doc(db,'mail',m._id));}catch(e){}
    }
    if(got.length){saveP();render();notify('🎴 Подарунок-картка!','Отримано: '+got.join(', '),5500);}
  }catch(e){}
}
...
function plotNeeds(p,el){
  ...
  return {
    water: pct>=0.30 && !p.watered,   // десь на 30% потребує поливу
    weeds: pct>=0.55 && !p.weeded,    // бур'ян з'являється на 55%
  };
}
...
function plotQuality(p){
  ...
  if(pct>=0.30&&!p.watered)q-=25;        // не полили вчасно
  if(pct>=0.55&&!p.weeded)q-=25;         // не пропололи
  if(p.fert)q+=30;                        // удобрено
  if(p.watered)q+=5;if(p.weeded)q+=5;
...
window.harvestPlot=function(i){
  ...
  let coins=Math.round((c.coins||0)*coinMul()*qMul);

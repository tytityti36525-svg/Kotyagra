  ...
  try{
    if(type==='players'){
      ...
      const players=snap.docs
        ...
        .filter(p=>p.nickname) // nickname is enough — catname may be missing on old accounts
...
async function renderFriendsPage(){
  ...
  if(!friends.length){
    list.innerHTML='<div class="loading-inline">У тебе поки немає друзів.<br><span style="font-size:.72rem">Відкрий профіль гравця в Рейтингу → Додати в друзі</span></div>';
    return;
  }
  list.innerHTML='<div class="loading-inline">Завантаження...</div>';
  try{
    const rows=await Promise.all(friends.map(async fid=>{
      try{
        const snap=await getDoc(doc(db,'players',fid));
        if(!snap.exists())return null;
        const p=snap.data();
        const av=(PET_TYPES[p.petType]||PET_TYPES.cat).emojis[0];
        return '<div class="friend-row" onclick="openPlayerProfile(\'''+fid+'\')">'+
          '<span class="friend-av">'+av+'</span>'+
          '<div style="flex:1">'+
            '<div class="friend-name">'+esc(p.catname||p.nickname||'Тваринка')+'</div>'+
            '<div class="friend-sub">@'+esc(p.nickname||'?')+' · Рів.'+( p.level||1)+' · 🦋'+(p.butterflies||0)+'</div>'+
          '</div>'+
          '<button class="small-btn" onclick="event.stopPropagation();removeFriend(\'''+fid+'\')"  >✕</button>'+
        '</div>';
      }catch(e){return null;}
    }));
    const html=rows.filter(Boolean).join('');
    list.innerHTML=html||'<div class="loading-inline">Профілі не знайдено</div>';
  }catch(e){list.innerHTML='<div class="loading-inline">Помилка</div>';}
}
...
window.searchPlayer=async function(){
  ...
  try{
    ...
    if(snap.empty){res.innerHTML='<div class="loading-inline">Гравця не знайдено</div>';return;}
    res.innerHTML=snap.docs.map(d=>{
      ...
      return '<div class="friend-row">'+
        ...
        '</div>'+
        (isMe?'<span style="font-size:.7rem;color:var(--tl)">Це ти</span>':
         already?'<span style="font-size:.7rem;color:var(--gn)">✅ В друзях</span>':
         '<button class="green-btn" style="padding:6px 10px;font-size:.72rem" onclick="addFriendById(\'''+d.id+'\')">+Додати</button>')+
      '</div>';
    }).join('');
  }catch(e){res.innerHTML='<div class="loading-inline">Помилка пошуку: '+esc(e.message)+'</div>';}
};
window.addFriendById=function(fid){
  ...
  notify('👥 Додано!','Гравця додано у друзі!');
  renderFriendsPage();
  const r=$('friend-search-result');if(r)r.innerHTML='';
  const i=$('friend-search-inp');if(i)i.value='';
};

...
// ── Config ────────────────────────────────────────────
const FB = {
  apiKey:["AIzaSyB0","XJ50di5q","f45qBC23","QKMuZJQo","rS593S0"].join(""),
...
let authMode = 'login', ratingTab = 'players';
let shopFilter = 'food';
...
function mkPlayer(nickname, catname, petType='cat') {
  return {
    ...
    inbox:[{id:1,from:'КотяГра',subj:'Ласкаво просимо!',
      body:'Привіт! Виховуй свого котика '+catname+'!',
...
function setErr(msg) {
  const el = $('auth-err');
  if (el) { el.textContent = msg; el.style.color = '#d32f2f'; }
}
function resetBtn() {
  const b = $('auth-btn');
...
window.switchTab = function(m) {
  ...
  const fields = ['a-name','a-cat','a-pettype'];
  fields.forEach(id => { const el=$(id); if(el) el.style.display = m==='reg' ? 'block' : 'none'; });
  resetBtn();
  setErr('');
};
...
window.doAuth = async function() {
  ...
  try {
    ...
    alert('Firebase: '+(msgs[e.code]||e.code+'\n'+e.message));
  }
};
  ...
  if (local) {
    ...
    if (P) {
      ...
      getDoc(doc(db,'players',uid)).then(s => {
        if (s.exists()) {
          const r = sanitize(s.data());
          if ((r.level||1) >= (P.level||1)) { P=r; render(); }
...
function startGame() {
  try {
    ...
    else if (P.walk) { P.walk=null; saveP(); }
    saveP();
    updateMailBadge();
  } catch(e) {
...
function startOnline() {
  const update = async () => {
    try {
      ...
      const cnt = s.docs.filter(d=>(d.data().t||0)>Date.now()-3*60*1000).length;
      if ($('s-on')) $('s-on').textContent=cnt;
      if ($('chat-on')) $('chat-on').textContent=cnt;
    } catch(e) {}
  };
  update();
  onlineInt = setInterval(update, 45000);
}
...
function startDecay() {
  ...
  decayInt = setInterval(() => {
    if (!P) return;
    if (!P.sleeping) {
      P.hunger = cl((P.hunger||0)-2);
      P.thirst = cl((P.thirst||0)-3);
      P.fun    = cl((P.fun||0)-1);
      P.energy = cl((P.energy||0)-1);
    } else {
      P.energy = cl((P.energy||0)+2);
    }
    if (Math.random() > 0.8) P.coins = (P.coins||0)+1;
    render();
  }, 4000);
}
...
function startSleepTimer() {
  ...
  const tick = () => {
    ...
    if (cd) cd.textContent='⏱ '+Math.floor(rem/60000)+'хв '+String(Math.floor((rem%60000)/1000)).padStart(2,'0')+'с';
    const bar = $('sleep-bar');
...
function gainXP(n) {
  ...
  if (P.xp >= cap) {
    ...
    notify('🎉 Рівень '+(P.level)+'!','Новий рівень! +'+P.level*10+'🪙');
...
window.petCat = function() {
  ...
  P.hearts=cl((P.hearts||0)+1,0,999999);P.fun=cl((P.fun||0)+5);
  gainXP(2);notify('😻 Муркоче!','+1❤️ +5😊');
};
window.act = function(type) {
  if(P.sleeping&&type!=='sleep'){notify('💤 Спить','Зачекайте...');return;}
  switch(type){
    case'feed':
      if((P.hunger||0)>=100){notify('😋 Ситий!','Вже не хоче їсти.');return;}
      P.hunger=cl((P.hunger||0)+25);gainXP(3);notify('🍖 Смачно!','+25🍖');trackTask('feedCnt');break;
    case'water':
      if((P.thirst||0)>=100){notify('💧 Напоєний!','');return;}
      P.thirst=cl((P.thirst||0)+30);gainXP(2);notify('💧 Освіжився!','+30💧');trackTask('waterCnt');break;
    case'play':
...
function finishWalk(){
  ...
  P.walkCoins=(P.walkCoins||0)+coins;
  gainXP(P.walk.xp);
  notify('🏠 Повернувся!','🪙 '+coins);trackTask('walkCnt');trackTask('walkTotal');
...
async function loadRatings(type) {
  ...
  try {
    ...
    if(type==='beauty') {
      ...
      all.sort((a,b)=>((b.level||1)*1000+(b.butterflies||0))-((a.level||1)*1000+(a.butterflies||0)));
    }
    const medals=['🥇','🥈','🥉'];
    rl.innerHTML='<div class="rating-count-info">👥 Гравців: '+all.length+'</div>'
      +all.map((p,i)=>{
        ...
        return '<div class="rating-row '+(isMe?'rating-row-me':'')+'" onclick="openProfile(\''+p._id+'\')">'
          ...
          +'<span class="rav">🐱</span>'
          +'<div style="flex:1">'
            +'<div class="rname">'+esc(p.catname||p.nickname||'Тваринка')+(isMe?' 🌟':'')+'</div>'
    ...
    rl.innerHTML='<div class="loading-inline">Помилка: '+esc(e.message)+'</div>';
  }
}
...
window.openProfile = async function(pid) {
  try {
    ...
    const p=snap.data();
    alert(
      '🐱 '+esc(p.catname||'Тваринка')+'\n'+
      '@'+esc(p.nickname||'?')+'\n'+
      'Рівень: '+(p.level||1)+'\n'+
      '🦋 Краса: '+(p.butterflies||0)+'\n'+
      '🏆 Перемог: '+(p.showWins||0)
    );
  } catch(e) {}
};
...
async function loadClubs(){
  ...
  try{
    ...
    const clubs=snap.docs.map(d=>({...d.data(),_id:d.id})).sort((a,b)=>(b.level||1)-(a.level||1));
    cb.innerHTML=clubs.map(c=>
      '<div class="club-list-item" onclick="joinClub(\''+c._id+'\')">'+
...
async function loadClubData(){
  ...
  try{
    ...
    $('clubs-in-club').style.display='block';
    $('clubs-no-club').style.display='none';
    S('cl-emblem',c.icon||'🏰');S('cl-name',c.name||'Клуб');
    const stars=Math.min(7,c.stars||1);
    S('cl-stars','★'.repeat(stars)+'☆'.repeat(Math.max(0,7-stars)));
    const isDir=c.directorUid===uid;
    S('cl-role-badge',isDir?'👑 Директор':'👤 Учасник');
    S('cl-desc',c.description||'—');S('cl-founded',c.founded||'—');
    S('cl-level',c.level||1);S('cl-mc',c.memberCount||1);
    S('cl-pc',(c.piggyCoins||0));S('cl-ph',(c.piggyHearts||0));
    const dr=$('club-director-row');if(dr)dr.style.display=isDir?'block':'none';
    const mSnap=await getDocs(query(collection(db,'clubs',P.clubId,'members'),limit(50)));
    const roles={director:'Директор',deputy:'Зам.',curator:'Куратор',member:'Учасник'};
    $('cl-members').innerHTML=mSnap.docs.map(md=>{
      ...
      return '<div class="member-row">'+
        ...
        '<span class="member-role">'+(roles[m.role||'member'])+'</span>'+
        '</div>';
    }).join('');
...
window.createClub = async function(){
  ...
  if(!name||name.length<3){notify('📝 Назва','Мінімум 3 символи!');return;}
  ...
  try{
    ...
    await setDoc(doc(db,'clubs',ref.id,'members',uid),{
      ...
      role:'director',points:0,joinedAt:new Date().toISOString()
    });
    P.clubId=ref.id;
    if($('new-club-name'))$('new-club-name').value='';
    if($('new-club-desc'))$('new-club-desc').value='';
    saveP();notify('🏆 Клуб створено!',name+' · ти Директор!');
    addLog('Клуб "'+name+'" створено!');
    renderClubs();
  }catch(e){P.coins+=50;notify('❌ Помилка',e.message);}
...
window.joinClub = async function(clubId){
  ...
  try{
    ...
    await setDoc(doc(db,'clubs',clubId,'members',uid),{
      uid:String(uid),nickname:String(P.nickname||'?'),
      catname:String(P.catname||'Тваринка'),
      role:'member',points:0,joinedAt:new Date().toISOString()
    });
    await updateDoc(doc(db,'clubs',clubId),{memberCount:increment(1)});
    P.clubId=clubId;saveP();
    notify('🎈 Вступив!','Клуб: '+cname);
    renderClubs();
  }catch(e){notify('❌ Помилка',e.message);}
};
...
window.leaveClub = async function(){
  ...
  try{
    ...
    P.clubId=null;saveP();notify('👋','Вийшов з клубу');renderClubs();
...
window.donateClub = async function(){
  ...
  try{
    ...
    notify('🐷 Копилка!','🪙'+amt+' передано!');loadClubData();render();saveP();
  }catch(e){P.coins+=amt;}
};
  ...
  try{
    ...
    chatUnsub=onSnapshot(q,snap=>{
      if(!loaded){
        ...
        docs.forEach(d=>{
          const m=d.data();
          addMsg(d.id,m.nickname,m.catname,m.text,m.uid===uid,m.ts);
        });
        box.scrollTop=box.scrollHeight;
      }else{
        snap.docChanges().forEach(ch=>{
          if(ch.type==='added'&&!document.getElementById('cm-'+ch.doc.id)){
            const m=ch.doc.data();
            addMsg(ch.doc.id,m.nickname,m.catname,m.text,m.uid===uid,m.ts);
            box.scrollTop=box.scrollHeight;
          }
        });
      }
    },e=>console.warn('chat:',e));
  }catch(e){if($('chat-msgs'))$('chat-msgs').innerHTML='<div class="loading-inline">Помилка чату</div>';}
}
...
function addMsg(docId,nick,catname,text,mine,ts){
  ...
  let t='';
  try{t=ts&&ts.toDate?ts.toDate().toLocaleTimeString('uk-UA',{hour:'2-digit',minute:'2-digit'}):now();}catch(e){t=now();}
  div.innerHTML=
    '<div class="msg-sender'+(mine?' msg-sender-mine':'')+'">'+
      esc(catname||nick||'?')+
      '<span style="font-size:.62rem;opacity:.7"> @'+esc(nick||'?')+'</span>'+
    '</div>'+
...
function renderPetPage() {
  if (!P) return;
  const nm = P.catname||'Тваринка';
  // Fix: show catname everywhere on pet page
  const elems = ['pet-cat-name','ps-name'];
  elems.forEach(id => { const el=document.getElementById(id); if(el) el.textContent=nm; });
  const lvl=document.getElementById('ps-lv'); if(lvl) lvl.textContent=(P.level||1)+' рівень';
  const beauty=document.getElementById('ps-beauty'); if(beauty) beauty.textContent=(P.butterflies||0);
  const coins=document.getElementById('ps-coins'); if(coins) coins.textContent=(P.coins||0);
  const hearts=document.getElementById('ps-hearts'); if(hearts) hearts.textContent=(P.hearts||0);
  const club=document.getElementById('ps-club'); if(club) club.textContent='Клуб: '+(P.clubId?'Є':'—');
  const days=Math.floor((Date.now()-new Date(P.createdAt||Date.now()).getTime())/86400000);
  const gd=document.getElementById('ps-gamedays'); if(gd) gd.textContent=days;
  const xpEl=document.getElementById('ps-xp'); if(xpEl) xpEl.textContent=(P.xp||0);
  const cap=xpCap(P.level||1);
  const capEl=document.getElementById('ps-cap'); if(capEl) capEl.textContent=cap;
  const xpbar=document.getElementById('ps-xpbar');
  if(xpbar) xpbar.style.width=((P.xp||0)/cap*100)+'%';
  // Unread mail
  const cnt=(P.inbox||[]).filter(m=>!m.read).length;
  const mc=document.getElementById('pm-mail-cnt');
  if(mc) mc.textContent=cnt>0?cnt+' непрочитаних':'';
}

// ── Training ─────────────────────────────────────────
const TRAIN_SKILLS = [
  {key:'clothes',icon:'👗',name:'Одяг',desc:'Одяг дає більше краси'},
  {key:'access', icon:'👑',name:'Аксесуари',desc:'Аксесуари дають більше краси'},
  {key:'jewel',  icon:'💍',name:'Прикраси',desc:'Прикраси дають більше краси'},
];

function renderTrain() {
  if(!P)return;
  const gl=document.getElementById('t-gl'); if(gl) gl.textContent=P.glamour||0;
  const bu=document.getElementById('t-bu'); if(bu) bu.textContent=P.butterflies||0;
  const c=document.getElementById('train-items');if(!c)return;
  if(!P.skills)P.skills={clothes:0,access:0,jewel:0};
  c.innerHTML=TRAIN_SKILLS.map(({key,icon,name,desc})=>{
    const lv=(P.skills[key]||0);
    const pct=Math.round(lv/80*100);
    return '<div class="train-item">'+
      '<span class="train-icon">'+icon+'</span>'+
      '<div style="flex:1">'+
        '<div class="train-name">'+name+' <small style="color:var(--tl)">Рів.'+lv+'/80</small></div>'+
        '<div class="train-desc">'+desc+'</div>'+
        '<div class="lvl-mini-track"><div class="lvl-mini-fill" style="width:'+pct+'%"></div></div>'+
        '<button class="train-btn" onclick="trainSkill(''+key+'')">Тренувати ❤️ 150</button>'+
      '</div>'+
    '</div>';
  }).join('');
}
...
window.trainSkill = function(sk) {
  ...
  if((P.skills[sk]||0)>=80){notify('✅ Максимум','Цей навик максимальний!');return;}
  P.hearts=Math.max(0,(P.hearts||0)-150);
  P.skills[sk]=(P.skills[sk]||0)+1;
  P.glamour=(P.glamour||0)+1;
  P.butterflies=(P.butterflies||0)+1;
  gainXP(8);
  const nm={clothes:'Одяг',access:'Аксесуари',jewel:'Прикраси'}[sk];
  notify('🏋️ Тренування!',nm+' рівень '+P.skills[sk]+' · +🦋1');trackTask('trainCnt');
...
const TASKS_DEF = [
  // Daily
  {id:'d_feed', cat:'daily', icon:'🍖', title:'Нагодуй тваринку',  goal:3,  track:'feedCnt',  reward:{coins:20,xp:15}},
  {id:'d_water',cat:'daily', icon:'💧', title:'Напій тваринку',    goal:3,  track:'waterCnt', reward:{coins:15,xp:10}},
  {id:'d_play', cat:'daily', icon:'🎾', title:'Пограй',            goal:2,  track:'playCnt',  reward:{coins:25,xp:20}},
  {id:'d_walk', cat:'daily', icon:'🌳', title:'Прогулянка',        goal:1,  track:'walkCnt',  reward:{coins:50,xp:30}},
  {id:'d_chat', cat:'daily', icon:'💬', title:'Напиши в чат',      goal:3,  track:'chatCnt',  reward:{coins:15,xp:10}},
  // Weekly
  {id:'w_train',cat:'weekly',icon:'🏋️',title:'Тренуй навик 5 разів',goal:5,track:'trainCnt',reward:{coins:150,xp:80}},
  {id:'w_walk5',cat:'weekly',icon:'🌲', title:'Зроби 5 прогулянок', goal:5, track:'walkTotal',reward:{coins:300,xp:150}},
  // Achievements
...
function initTasks(){
  if(!P.tasks){
    P.tasks={
      ...
      lastWeekly:getMondayKey(),
    };
  }
  // Reset daily
  const today=new Date().toDateString();
  if(P.tasks.lastDaily!==today){
    TASKS_DEF.filter(t=>t.cat==='daily').forEach(t=>{
      delete P.tasks.completed[t.id];
  ...
  if(P.tasks.lastWeekly!==wk){
    TASKS_DEF.filter(t=>t.cat==='weekly').forEach(t=>{
      delete P.tasks.completed[t.id];
      P.tasks.progress[t.track]=0;
    });
...
function trackTask(key,amt=1){
  ...
  TASKS_DEF.forEach(t=>{
    ...
    if(getProgress(t)>=t.goal){
      ...
      notify('✅ Завдання!',t.icon+' '+t.title+' — забери нагороду!',4000);
    }
  });
}
...
function renderTasks(){
  ...
  const tasks=TASKS_DEF.filter(t=>t.cat===tasksTab);
  const info=document.getElementById('tasks-reset-info');
  if(info){
    if(tasksTab==='daily')info.textContent='🔄 Оновлюється щодня опівночі';
    else if(tasksTab==='weekly')info.textContent='🔄 Оновлюється щопонеділка';
    else info.textContent='🏅 Постійні досягнення';
  }
  list.innerHTML=tasks.map(t=>{
    ...
    const rwd=Object.entries(t.reward).map(([k,v])=>({coins:'🪙'+v,xp:'⭐'+v,hearts:'❤️'+v})[k]).join(' ');
...
window.claimTask = function(taskId){
  ...
  const prog=getProgress(t);
  if(prog<t.goal){notify('❌','Спочатку виконай завдання!');return;}
  if(t.once&&P.tasks.claimed[taskId]){notify('✅','Вже отримано!');return;}
  if(t.reward.coins)P.coins=(P.coins||0)+t.reward.coins;
  if(t.reward.hearts)P.hearts=Math.min(999999,(P.hearts||0)+t.reward.hearts);
  if(t.reward.xp)gainXP(t.reward.xp);
  const rwd=Object.entries(t.reward).map(([k,v])=>({coins:'🪙'+v,xp:'⭐'+v,hearts:'❤️'+v})[k]).join(' ');
  notify('🎁 Нагорода!',t.icon+' '+rwd,4000);
  addLog('Завдання "'+t.title+'" виконано! '+rwd);
  if(t.once){P.tasks.claimed[taskId]=true;}
  else{P.tasks.completed[taskId]=false; P.tasks.progress[t.track]=0;}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(window._authQueue==='login') { window._authQueue=null; window.doAuth(); }

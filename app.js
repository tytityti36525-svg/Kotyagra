...
function applyMenuIcons(){
  if(!window.MENU_ICONS)return;
  document.querySelectorAll('.hm-img[data-icon]').forEach(img=>{
    const k=img.getAttribute('data-icon');
    if(MENU_ICONS[k]&&img.src!==MENU_ICONS[k])img.src=MENU_ICONS[k];
  });
}

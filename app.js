...
function coinMul(){let m=(bonusActive('coin2')||bonusActive('vip'))?2:1;if(P&&P._clubBuffUntil&&P._clubBuffUntil>Date.now())m*=1.25;return m*weMul('coin')*(1+skillEff('coinPct')/100)*(1+(typeof mansionBonus==='function'?mansionBonus():0)/100)*(1+((P&&P._clubPerk)||0)/100)*(1+(typeof roomEff==='function'?roomEff('coinPct'):0)/100);}
function gainXP(n){
  n=Math.round(n*xpMul()*personalXpFactor()*(1+(typeof tempEff==='function'?tempEff('xp'):0))*(1+(typeof roomEff==='function'?roomEff('xpPct'):0)/100));
  P.xp=(P.xp||0)+n;

...
onAuthStateChanged(auth,async user=>{
  ...
  try{
    ...
    if(s.exists()){
      ...
      // Always re-save to Firestore to ensure data is fresh and complete
      setTimeout(()=>setDoc(doc(db,'players',uid),san(P),{merge:true}).catch(()=>{}),1000);
    }else{
      // New player — use email prefix as nickname if no displayName
      const emailNick=(user.email||'').split('@')[0].replace(/[^a-zA-Zа-яА-Я0-9_]/g,'').slice(0,20)||'Гравець';
      const n=(user.displayName||emailNick).replace(/\s+/g,'_').slice(0,20);
      P=mkPlayer(n,'Котик','cat');
      await setDoc(doc(db,'players',uid),san(P)).catch(()=>{});
    }
  }catch(e){
    console.warn('Firestore read error:',e.code);
    // Try to create player doc anyway
    const n=(user.email||'').split('@')[0].slice(0,20)||'Гравець';
    P=mkPlayer(n,'Котик','cat');

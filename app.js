...
async function refreshUnread(){
  try{
    ...
    window._mailUnread=snap.docs.filter(d=>{const m=d.data();return m.read===false&&m.type!=='friendreq'&&m.type!=='friendaccept'&&m.type!=='refreward'&&m.type!=='visit';}).length;
    updateMailBadge();
  }catch(e){}
}
...
window.goPage=function(id){
  ...
  if(id==='settings'&&typeof refreshInstallUI==='function'){refreshInstallUI();const sb=$('sound-toggle-btn');if(sb&&P)sb.textContent=P.sound?'🔊 Звук: увімкнено':'🔇 Звук: вимкнено';}
...
async function loadMail(kind){
  ...
  const arr=snap.docs.map(d=>({_id:d.id,...d.data(),time:tsToStr(d.data().ts)}))
    .filter(m=>m.type!=='friendreq'&&m.type!=='friendaccept'&&m.type!=='refreward'&&m.type!=='visit') // службові — окремо
...
function updateVisitBtn(){
  const b=$('pm-visit-btn');if(!b||!_profileTarget)return;

...
function mkPlayer(nickname,catname,petType='cat'){
  return{
    ...
    mansion:0,clean:100,bank:0,bankSince:0,friendXp:{},
    riddleDay:'',rouletteDay:'',rouletteSpins:0,loginCal:{streak:0,last:'',claimed:[]},music:false,
    landmarks:{},title:'',
...
function coinMul(){let m=(bonusActive('coin2')||bonusActive('vip'))?2:1;if(P&&P._clubBuffUntil&&P._clubBuffUntil>Date.now())m*=1.25;return m*weMul('coin')*(1+skillEff('coinPct')/100)*(1+(typeof mansionBonus==='function'?mansionBonus():0)/100);}
function gainXP(n){
...
window.act=function(type){
  ...
  switch(type){
    case'feed':
      ...
      P.hunger=cl((P.hunger||0)+25);P.hearts=(P.hearts||0)+Math.round(20*(1+skillEff('heartPct')/100));gainXP(3);notify('🍖 Смачно!','+25🍖 +20❤️');
  ...
  c.innerHTML=(SHOP[cat]||[]).map(item=>{
    ...
    return '<div class="shop-item-card">'
      ...
      +(owned?'<button class="si-buy" disabled>✅ Є</button>'
        :'<button class="si-buy" '+((P?.coins||0)<price?'disabled':'')+' onclick="buyItem(\''+item.id+'\',\''+cat+'\')">'+priceLbl+'</button>')
    +'</div>';
...
const FISH_CATCH=[
  ...
  {icon:'🐟',name:'Карась',      cMin:12,cMax:24,    w:26, ing:'karas'},
  {icon:'🐠',name:'Окунь',       cMin:28,cMax:48,    w:22, ing:'okun'},
  {icon:'🐡',name:'Короп',       cMin:48,cMax:80,    w:16, ing:'korop'},
  {icon:'🦐',name:'Креветка',    bf:5,               w:9,  ing:'shrimp'},
  {icon:'🦈',name:'Рідкісна рибина',cMin:100,cMax:170,bf:5,w:7, ing:'rare'},
  {icon:'💎',name:'Фрагмент каменя',gem:1,           w:5},
  {icon:'👢',name:'Скарб на дні', cMin:180,cMax:320, w:5},
];
let fishTicker=null;
...
window.fishingAction=function(){
  ...
  let coins=0,parts=[];
  if(c.cMin!=null){coins=c.cMin+Math.floor(Math.random()*(c.cMax-c.cMin+1));coins=Math.round(coins*coinMul()*(typeof weatherToday==='function'?weatherToday().fishMul:1)*weMul('fish')*(1+skillEff('fishPct')/100));P.coins=(P.coins||0)+coins;parts.push('🪙'+coins);}
...
function renderKitchen(){
  ...
  if(pg){
    ...
    pg.innerHTML=owned.length?owned.map(k=>
      '<div class="pantry-item"><span class="pantry-ic">'+INGREDIENTS[k].icon+'</span>'
  ...
  rl.innerHTML=RECIPES.map(r=>{
    ...
    return '<div class="recipe-row'+(ok?'':' locked')+'">'
      +'<span class="recipe-ic">'+r.icon+'</span>'
...
function renderFamily(){
  ...
  html+='</div>';
  // 2) Шлюб
  html+='<div class="card"><div class="sec-title">💍 Шлюб</div>';
  if(P.spouseUid){
  ...
  html+='</div>';
  // 3) Вхідні пропозиції
  html+='<div class="card"><div class="sec-title">💌 Пропозиції руки</div><div id="marriage-proposals"><div class="loading-inline">Завантаження...</div></div></div>';
  // 4) Малюк
  html+='<div class="card"><div class="sec-title">👶 Малюк</div>';
  if(P.baby){
...
window.bathePet=function(){
  ...
  showEmotion('play',2400);if(typeof sfx==='function')sfx('success');
...
function bankInterest(){
  if(!P.bank||!P.bankSince)return 0;
  const days=Math.min(BANK_MAXDAYS,(Date.now()-P.bankSince)/86400000);
  return Math.floor(P.bank*BANK_RATE*days);
}
function renderBank(){
  const box=$('bank-body');if(!box)return;
  const intr=bankInterest();
  box.innerHTML='<div class="card" style="text-align:center"><div style="font-size:2.6rem">🏦</div>'
    +'<div class="sec-title">Банк КотяГри</div>'
    +'<div class="fam-note">На депозиті: 🪙<b>'+(P.bank||0)+'</b><br>Нараховано відсотків: 🪙<b>'+intr+'</b><br>+3% на день, максимум 7 днів</div>'
    +'<div style="display:flex;gap:8px;margin-top:12px"><input class="input-field" id="bank-amount" type="number" placeholder="Сума" style="flex:1">'
    +'<button class="green-btn" onclick="bankDeposit()" style="flex:0 0 auto">Покласти</button></div>'
...
window.bankDeposit=function(){
  ...
  // нараховуємо поточні відсотки перед новим внеском
  const intr=bankInterest();if(intr>0){P.bank=(P.bank||0)+intr;}
  P.coins-=v;P.bank=(P.bank||0)+v;P.bankSince=Date.now();
  notify('🏦 Покладено!','На депозиті 🪙'+P.bank);render();saveP();renderBank();
};
window.bankWithdraw=function(){
  if(!P.bank){notify('🏦','Депозит порожній');return;}
  const intr=bankInterest();const total=(P.bank||0)+intr;
  P.coins=(P.coins||0)+total;P.bank=0;P.bankSince=0;
  if(typeof sfx==='function')sfx('coin');
...
const RIDDLES=[
  ...
  {q:'Що можна зловити, але не втримати?',a:['Застуду','Рибу','М\'яч'],c:0},
...
window.spinRoulette=function(){
  ...
  // невеликі виграші/трати: 0x, 0.5x, 1x(повернення), 1.5x, 2x
  const r=Math.random();let mult,txt;
  if(r<0.25){mult=0;txt='😿 Програш';}
  else if(r<0.5){mult=0.5;txt='🙂 Половина назад';}
  else if(r<0.75){mult=1;txt='😐 Повернення';}
  else if(r<0.93){mult=1.5;txt='😺 Виграш x1.5';}
  else{mult=2;txt='🎉 Виграш x2!';}
  const win=Math.round(bet*mult);P.coins+=win;
  if(typeof sfx==='function')sfx(mult>=1.5?'coin':'click');
  notify('🎰 '+txt,(win>0?'+🪙'+win:'нічого')+' · ставок сьогодні: '+P.rouletteSpins+'/'+ROULETTE_MAX,4000);
...
function renderRoulette(){
  ...
  box.innerHTML='<div class="fam-note">Ставка 🪙50. Шанс виграти до x2. Ставок сьогодні: '+Math.max(0,left)+'/'+ROULETTE_MAX+'</div>'
    +'<button class="orange-btn" '+(left<=0?'disabled':'')+' onclick="spinRoulette()" style="margin-top:8px;width:100%">🎰 Крутити (ставка 🪙50)</button>';
}
function renderGames(){renderRiddle();renderRoulette();}
...
// ════════ ФОНОВА МУЗИКА (процедурна, без файлів) ════════
let _music=null;
function startMusic(){
  if(_music)return;
  try{
    const ac=_actx||new (window.AudioContext||window.webkitAudioContext)();_actx=ac;
    const master=ac.createGain();master.gain.value=0.08;master.connect(ac.destination);
    // спокійна послідовність (до-мажор арпеджіо)
    const notes=[261.63,329.63,392.0,523.25,392.0,329.63,293.66,349.23,440.0,349.23];
    let i=0;
    const tick=()=>{
      if(!_music)return;
      const o=ac.createOscillator(),g=ac.createGain();o.type='sine';o.frequency.value=notes[i%notes.length];
      o.connect(g);g.connect(master);
      g.gain.setValueAtTime(0,ac.currentTime);g.gain.linearRampToValueAtTime(0.5,ac.currentTime+0.1);
      g.gain.linearRampToValueAtTime(0,ac.currentTime+0.9);
      o.start();o.stop(ac.currentTime+1);
      i++;
    };
    tick();_music=setInterval(tick,900);
  }catch(e){}
}
function stopMusic(){if(_music){clearInterval(_music);_music=null;}}

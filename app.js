...
function coinMul(){let m=(bonusActive('coin2')||bonusActive('vip'))?2:1;if(P&&P._clubBuffUntil&&P._clubBuffUntil>Date.now())m*=1.25;return m*weMul('coin')*(1+skillEff('coinPct')/100)*(1+(typeof mansionBonus==='function'?mansionBonus():0)/100)*(1+((P&&P._clubPerk)||0)/100);}
function gainXP(n){
...
window.openClubLeaderboard=async function(){
  ...
  try{
    ...
    box.innerHTML=clubs.map((c,i)=>{
      ...
      return '<div class="club-rank-row'+(mine?' mine':'')+'">'
        ...
        +'<div class="club-rank-sub">Рівень '+(c.level||1)+' · ⭐'+(c.xp||0)+' · 👥'+(c.memberCount||1)+'</div></div></div>';
    }).join('');
...
window.spinRoulette=function(){
  ...
  if(r<0.25){mult=0;txt='Програш';emo='😿';}
  else if(r<0.5){mult=0.5;txt='Половина назад';emo='🙂';}
  else if(r<0.75){mult=1;txt='Повернення ставки';emo='😐';}
  else if(r<0.93){mult=1.5;txt='Виграш ×1.5';emo='😺';}
  else{mult=2;txt='Виграш ×2!';emo='🎉';}

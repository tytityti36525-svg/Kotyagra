...
function startGame(){
  ...
  // Resume sleep if still active
  if(P.sleeping&&P.sleepStart&&P.sleepDur){
    const elapsed=Date.now()-P.sleepStart;
    if(elapsed<P.sleepDur){$('cat-emoji').textContent='😴';startSleepTimer();}
    else{wakeUp();}
  }
  updateMailBadge();
}
...
function calcBeauty(){
  ...
  // house bonus from star system
  const sd=getHouseStarData();if(sd)b+=sd.beauty;
  return b;
}
...
// Each star requires 4 components upgraded. 5 stars total = house levels 1-10
...
// Cost per component upgrade by current house star level
const HOUSE_COMP_COSTS = [50, 120, 280, 600, 1200]; // per star tier (0-indexed = star 1 components cost this)
// House visual/name by star
const HOUSE_STARS = [
  {star:0,emoji:'🏚️',name:'Руїни'},
...
function getHouseStar(){
  ...
  // Star = min level of all components (0-5)
  return Math.min(...HOUSE_COMPONENTS.map(c=>comps[c.key]||0));
}
function getHouseStarData(){
  const star=getHouseStar();
  return HOUSE_STARS[Math.min(star,HOUSE_STARS.length-1)];
}

// КотяГра app.js — v6 (cache bust 1781002559)
...
async function loadClubs(){
  ...
  try{
    console.log('[clubs] loading clubs, uid=',uid);
    const snap=await getDocs(collection(db,'clubs'));
    console.log('[clubs] fetched:',snap.size);
    if(snap.empty){
...
async function loadRatings(type){
  ...
  try{
    if(type==='players'){
      ...
      console.log('[ratings] total docs:',snap.size);
      const players=snap.docs
        .map(d=>({...d.data(),_id:d.id}))
        .filter(p=>p._id)
        .sort((a,b)=>((b.level||1)*1000+(b.butterflies||0))-((a.level||1)*1000+(a.butterflies||0)));
      console.log('[ratings] players:',players.length, players.map(p=>p.nickname));
      if(!players.length){
        rl.innerHTML='<div class="loading-inline">Поки немає гравців<br><small>Всього в базі: '+snap.size+' записів</small></div>';
        return;
      }
      // show count for debug
      rl.innerHTML='<div style="font-size:.65rem;color:var(--tl);text-align:center;padding:2px 0 6px">Гравців у базі: '+players.length+'</div>';

...
window.doAuth=async function(){
  ...
  try{
    ...
    const msgs={
      ...
      'auth/operation-not-allowed':'Email/пароль не увімкнено у Firebase!',
      'auth/configuration-not-found':'Firebase не налаштований!',
    };
    // Показуємо повну помилку на екрані щоб бачити на айфоні
    setErr(msgs[e.code]||('Помилка: '+(e.code||e.message||'невідома')));
  }
};
...
// ── BIND BUTTONS ──
// app.js — type="module", тому виконується після парсингу DOM
// DOMContentLoaded може вже вистрілити — використовуємо requestAnimationFrame як fallback
function bindButtons(){
  const bind=(id,fn)=>{const el=$(id);if(el){el.addEventListener('click',fn);}};
  bind('auth-btn',()=>window.doAuth());
  bind('tab-login',()=>window.switchTab('login'));
  bind('tab-reg',()=>window.switchTab('reg'));
  const pass=$('a-pass');
  if(pass)pass.addEventListener('keydown',e=>{if(e.key==='Enter')window.doAuth();});
  buildShop('food');
}

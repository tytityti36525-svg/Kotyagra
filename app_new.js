...
// ═══════════════════════════════════════════════════
// НОВІ ФУНКЦІЇ — додано до робочої версії
// Не змінюємо нічого вище, тільки додаємо знизу
// ═══════════════════════════════════════════════════

// ── SHOP розширений ──
// Перезаписуємо SHOP після того як він оголошений
Object.assign(SHOP,{
  clothes:[
    {id:'bow',    icon:'🎀',name:'Бантик',        desc:'+2🦋',cost:40, fn:()=>{_addWardrobe({id:'bow',    icon:'🎀',name:'Бантик',       type:'shirt',beauty:2});}},
    {id:'ribbon', icon:'🎗️',name:'Стрічка',        desc:'+3🦋',cost:60, fn:()=>{_addWardrobe({id:'ribbon', icon:'🎗️',name:'Стрічка',       type:'shirt',beauty:3});}},
    {id:'vest',   icon:'🧥',name:'Жилетка',        desc:'+4🦋',cost:80, fn:()=>{_addWardrobe({id:'vest',   icon:'🧥',name:'Жилетка',       type:'shirt',beauty:4});}},
    {id:'cape',   icon:'🦸',name:'Плащ',           desc:'+5🦋',cost:110,fn:()=>{_addWardrobe({id:'cape',   icon:'🦸',name:'Плащ',         type:'shirt',beauty:5});}},
    {id:'dress',  icon:'👸',name:'Коронна сукня',  desc:'+8🦋',cost:180,fn:()=>{_addWardrobe({id:'dress',  icon:'👸',name:'Коронна сукня',type:'shirt',beauty:8});}},
  ],
  accessories:[
    {id:'hat',    icon:'🎩',name:'Капелюх',  desc:'+2🦋',cost:45, fn:()=>{_addWardrobe({id:'hat',    icon:'🎩',name:'Капелюх', type:'hat',beauty:2});}},
    {id:'glasses',icon:'🕶️',name:'Окуляри',  desc:'+3🦋',cost:55, fn:()=>{_addWardrobe({id:'glasses',icon:'🕶️',name:'Окуляри', type:'hat',beauty:3});}},
    {id:'necklace',icon:'📿',name:'Намисто', desc:'+4🦋',cost:70, fn:()=>{_addWardrobe({id:'necklace',icon:'📿',name:'Намисто',type:'collar',beauty:4});}},
    {id:'ring',   icon:'💍',name:'Каблучка', desc:'+3🦋',cost:65, fn:()=>{_addWardrobe({id:'ring',   icon:'💍',name:'Каблучка',type:'ring',beauty:3});}},
    {id:'crown',  icon:'👑',name:'Корона',   desc:'+7🦋',cost:150,fn:()=>{_addWardrobe({id:'crown',  icon:'👑',name:'Корона',  type:'hat',beauty:7});}},
  ],
  food:[
    {id:'fish',  icon:'🐟',name:'Рибка',    desc:'+30🍖',       cost:8, fn:()=>{P.hunger=cl((P.hunger||0)+30);}},
    {id:'milk',  icon:'🥛',name:'Молоко',   desc:'+20💧',       cost:5, fn:()=>{P.thirst=cl((P.thirst||0)+20);}},
  ...
  gems:[
    {id:'frag_ruby',    icon:'🔴',name:'Рубін (фраг.)',   desc:'+1 фрагмент', cost:30,fn:()=>{_addGemFrag('ruby');}},
    {id:'frag_sapphire',icon:'🔵',name:'Сапфір (фраг.)',  desc:'+1 фрагмент', cost:30,fn:()=>{_addGemFrag('sapphire');}},
    {id:'frag_emerald', icon:'🟢',name:'Смарагд (фраг.)', desc:'+1 фрагмент', cost:30,fn:()=>{_addGemFrag('emerald');}},
    {id:'frag_citrine', icon:'🟡',name:'Цитрин (фраг.)',  desc:'+1 фрагмент', cost:25,fn:()=>{_addGemFrag('citrine');}},
  ],
  bonuses:[
    {id:'coin2x',     icon:'🪙',name:'Подвійні монети', desc:'x2 монети 10хв', cost:50, fn:()=>{_activateBonus('coin2x',600000);}},
    {id:'xp2x',       icon:'⭐',name:'Подвійний досвід',desc:'x2 XP 10хв',    cost:50, fn:()=>{_activateBonus('xp2x',600000);}},
...
function _addWardrobe(item){
  if(!P.wardrobe)P.wardrobe=[];
  if(P.wardrobe.length>=20){notify('👗','Шафа повна! Макс 20');return;}
  P.wardrobe.push({...item,acquired:new Date().toISOString()});
  P.butterflies=(P.butterflies||0)+item.beauty;
  notify('👗 '+item.name,'+'+item.beauty+'🦋');
}
function _addGemFrag(type){
  if(!P.gems)P.gems={ruby:0,sapphire:0,emerald:0,citrine:0,diamond:0};
  P.gems[type]=(P.gems[type]||0)+1;
  const names={ruby:'Рубін',sapphire:'Сапфір',emerald:'Смарагд',citrine:'Цитрин',diamond:'Діамант'};
  notify('💎 Фрагмент!',names[type]||type);
}
function _activateBonus(id,dur){
  if(!P.bonuses)P.bonuses={};
  P.bonuses[id]={expires:Date.now()+dur};
  notify('🎁 Бонус активовано!',id);
}
...
function renderGems(){
  ...
  if(list){
    list.innerHTML=Object.entries(GEM_DEFS).map(([key,g])=>{
      ...
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1.5px dashed var(--br)">'
        ...
        +'<div style="flex:1">'
          ...
          +'<div style="background:#ede3d0;border-radius:6px;height:6px;width:100%;overflow:hidden;margin-top:4px">'
            +'<div style="height:100%;border-radius:6px;background:'+g.color+';width:'+pct+'%;transition:width .3s"></div>'
          +'</div>'
        +'</div>'
  ...
  if(asm){
    asm.innerHTML=Object.entries(GEM_DEFS).map(([key,g])=>{
      const count=P.gems[key]||0;
      const can=count>=5;
      return '<button class="act-btn" '+(can?'onclick="assembleGem(\''+key+'\')"':'disabled')+' style="opacity:'+(can?1:.4)+'">'
...
function _getHouseStars(){
  if(!P.houseComponents)P.houseComponents={foundation:0,roof:0,walls:0,interior:0};
  return Object.values(P.houseComponents).reduce((s,v)=>s+(v||0),0);
}
function _getHouseStage(){
  const stars=_getHouseStars();
...
function renderHouseCard(){
  ...
  const sd=$('house-stars-display');
  if(sd)sd.textContent='★'.repeat(filled)+'☆'.repeat(5-filled);
  // Бонус краси від будинку
...
function _renderHouseModal(){
  ...
  const filled=Math.min(5,Math.floor(stars/4));
  const hs=$('hm-stars');if(hs)hs.textContent='★'.repeat(filled)+'☆'.repeat(5-filled);
...
window.upgradeHouseComp=function(key,cost){
  ...
  notify('🏗️ '+c.name+' покращено!','рів.'+P.houseComponents[key]);
  addLog(c.name+' → рів.'+P.houseComponents[key]);
  _renderHouseModal();renderHouseCard();render();saveP();
};
...
window.renderShow=function(){
  ...
  if(lb){
    lb.innerHTML='<div style="display:flex;align-items:center;gap:12px;background:linear-gradient(135deg,#fffbee,#fff3cc);border:2px solid var(--gold);border-radius:16px;padding:12px 14px;margin-bottom:8px">'
      +'<span style="font-size:2.2rem">'+lg.icon+'</span>'
      +'<div style="flex:1">'
        +'<div style="font-size:.9rem;font-weight:900;color:var(--gdk)">Ліга: '+lg.name+'</div>'
        +'<div style="font-size:.7rem;color:var(--tl);font-weight:700;margin-top:2px">Тижневий приз: 🪙'+lg.prize.coins+' ❤️'+lg.prize.hearts+'</div>'
        +(next?'<div style="font-size:.65rem;color:var(--tl);font-weight:700;margin-top:2px">До '+next.name+': ще '+(next.minBeauty-beauty)+' 🦋</div>'
          :'<div style="font-size:.65rem;color:var(--gold);font-weight:800;margin-top:2px">🏆 Максимальна ліга!</div>')
      +'</div>'
...
window.enterShow=async function(type){
  ...
  try{
    ...
    addLog('Виставка: '+type);
    _loadShowLeaderboard();
  }catch(e){notify('❌',e.message);}
  saveP();
};
async function _loadShowLeaderboard(){
  ...
  try{
    ...
    const medals=['🥇','🥈','🥉'];
    lb.innerHTML='<div style="font-size:.78rem;font-weight:900;color:var(--gdk);text-align:center;padding:6px 0 8px;border-bottom:1.5px dashed var(--br);margin-bottom:8px">🏅 Топ виставки</div>'
      +snap.docs.map((d,i)=>{const p=d.data();const isMe=d.id===uid;
        return '<div class="rating-row'+(isMe?' rating-row-me':'')+'">'
          +'<span style="font-size:.82rem;font-weight:900;color:var(--tl);min-width:24px">'+(medals[i]||i+1)+'</span>'
          +'<span style="font-size:1.2rem">'+_getLeague(p.beauty||0).icon+'</span>'
...
window.claimShowPrize=function(){
  const lg=_getLeague(P.butterflies||0);
...
window.levelUpClub=async function(){
  ...
  try{
    ...
    if(!snap.exists())return;
    const c=snap.data();
    if(c.directorUid!==uid){notify('❌','Тільки директор!');return;}
    const lv=c.level||1;
    const needed=lv*500;
...
// ── HOOK goPage — додаємо show/gems/pet без зміни оригіналу ──
const _goPageOrig=window.goPage;
window.goPage=function(id){
  _goPageOrig(id);
  if(id==='show')window.renderShow&&window.renderShow();
  if(id==='gems')renderGems&&renderGems();
  if(id==='pet')renderHouseCard&&renderHouseCard();
};

// ── HOOK finishWalk — каміння на прогулянках ──
const _finishWalkOrig=finishWalk;
finishWalk=function(){
  _finishWalkOrig();
  try{_dropGemOnWalk();}catch(e){}
};
